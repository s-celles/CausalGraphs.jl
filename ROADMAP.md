...
- [ ] **Project Setup:** Initialize the Julia package `CausalGraphs.jl` (if not already done). Set up `Project.toml`, `src/CausalGraphs.jl`, and `test/runtests.jl`.
- [ ] **Type Definitions:**
...
- [ ] **Core Graph Structure:** Implement `CauseEffectGraph` struct. It should store nodes, directed edges, and metadata.
- [ ] **Graph API:**
...
- [ ] **Tests:** Write unit tests for graph creation, node identity stability, and metadata attachment.

## Phase 2: Modeling Layer
**Goal:** Implement the ability to derive specific models from the underlying knowledge graph.

- [ ] **Model Structures:** Define `AbstractModel` and `Model` structs. A model references a base `CauseEffectGraph` and tracks a subset of its nodes/edges.
- [ ] **Model API:**
  - `add_model!(g, id; metadata...)`
  - `add_to_model!(model, node_or_edge)`
- [ ] **Model Composition & Abstraction:** Implement logic for submodels and alternative models for the same effect.
- [ ] **Tests:** Validate that multiple models can coexist and use subsets of the knowledge graph without modifying the underlying graph.
...
- [ ] **Cycle Detection:** Optional check for DAG property when requested.
- [ ] **Model Validation:** Check for unused causes or unsupported relationships within a model context.
...
- [ ] **MeasurementModel:** Implement a specialized `MeasurementModel <: AbstractModel`.
- [ ] **API Additions:**
...
- [ ] **Tests:** Recreate the GUM H.1 example structurally.

## Phase 5: Views and Serialization
**Goal:** Implement independent view extraction (Ishikawa, Tree) and I/O.

- [ ] **Serialization:** Implement `write_json(g, path)` / `read_json(path)` or TOML to ensure stable identity round-tripping.
- [ ] **View Structures:** Define intermediate structures for layouts (e.g., `IshikawaLayout`).
- [ ] **GraphViz / Text Representation:** Output a text-based tree or DOT format as a baseline visualization backend.
- [ ] **Tests:** Ensure serialization is deterministic and round-trips correctly.
